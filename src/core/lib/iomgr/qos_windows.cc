...
int grpc_qos_get_dscp(
  ...
  return AdjustValue(DSCP_NOT_SET, 0, 63, config.GetInt(GRPC_ARG_DSCP));
}
...
void grpc_qos_set_dscp(const grpc_winsocket* socket,
  ...
  // Get handle to the QoS subsystem
  ...
  int result =
      QOSAddSocketToFlow(handle, socket->socket, (grpc_sockaddr*)&address->addr,
